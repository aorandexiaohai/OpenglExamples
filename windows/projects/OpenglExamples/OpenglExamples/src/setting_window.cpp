...
SettingWindow::SettingWindow(void *window_handler)
{
    ...
    ImFont *font =
        io.Fonts->AddFontFromFileTTF("./imgui_font/simkai.ttf", 18.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());
    assert(font);
}
...
bool show_combobox(const std::string &slider_info_utf8, int &value, int &result, const std::vector<int> &v1,
                   const std::vector<const char *> &v2, const char *unique_label)
{
...
bool show_slider_float(const std::string &slider_info_utf8, float min_value, float max_value, float *value,
...
bool show_slider_float2(const std::string &slider_info_utf8, float min_value, float max_value, float *value,
                        const char *unique_label)
{
    auto label = slider_info_utf8 + ":" + toStr(min_value);
    ImGui::Text(label.c_str());
...
void SettingWindow::render()
{
    ...
    if (ImGui::CollapsingHeader(u8"Opengl示例列表"))
    {
        ...
        if (!vec_all_names_c.empty() &&
            ImGui::Combo(HIDE_LABEL, &select_item, vec_all_names_c.data(), (int)vec_all_names_c.size()))
        {
