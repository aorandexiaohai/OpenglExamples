...
#define ERROR_LOG_AND_ASSERT(x) {ERROR_LOG(x); assert(false); throw std::runtime_error("runtime_error");}
#define ERROR_LOG_AND_ASSERT_IF_FALSE(x) {if(!(x)){ERROR_LOG_AND_ASSERT(#x);}}
#define ERROR_LOG_AND_ASSERT_IF_TRUE(x) {if((x)){ERROR_LOG_AND_ASSERT(#x);}}
